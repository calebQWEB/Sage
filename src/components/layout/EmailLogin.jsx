...
const EmailLogin = ({setSignInModal}) => {
    const {login} = useAuth()
  ...
  return (
      <form onSubmit={onSubmitFunc} className="grid gap-5 items-center justify-center">
        ...
        <p className="text-linkMobile sm:text-link mt-10 cursor-pointer">Don't have an accout? <span className="text-secondary" onClick={() => setSignInModal(false)}>Sign Up</span></p>
      </form>
  )
}
